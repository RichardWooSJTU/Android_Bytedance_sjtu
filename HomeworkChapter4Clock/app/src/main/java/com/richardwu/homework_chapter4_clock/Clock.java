package com.richardwu.homework_chapter4_clock;
...
public class Clock extends View {
    ...
    private static final int UNIT_STEP_ANGLE = 30;
    ...
    public static final int MSG_TMP = 100;
    @Override
...
//        },1000);
        ...
        invalidate();
        handler1.sendEmptyMessageDelayed(MSG_TMP, 1000);
    ...
    private void drawHoursValues(Canvas canvas) {
        ...
        int r = mCenterX - (int) (mWidth * 0.12f);
        int yOffset = 15;
        int hourValue;
        for (int i = 90; i > -270; i -= UNIT_STEP_ANGLE /* Step */) {
            int ii = 90 - i;
            if (ii == 0) hourValue = 12;
            else hourValue = ii / UNIT_STEP_ANGLE;
            int x = (int) (mCenterX + r * Math.cos(Math.toRadians(i)));
            int y = (int) (mCenterX - r * Math.sin(Math.toRadians(i)));
            canvas.drawText(String.valueOf(hourValue),x,y+yOffset,paint);
        }
    ...
    private void drawNeedles(final Canvas canvas) {
        ...
        // 画分针
        // todo 2: 画分针
        int secondsOfHour = nowSeconds / 60;
        drawPointer(canvas, 1, nowMinutes + secondsOfHour);
        // 画时针
